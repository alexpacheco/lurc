...
```{r echo=FALSE}
sol <- tribble(~"Nodes",~"CPU Type",~"CPU Speed (GHz)",~"CPUs",~"GPUs",~"CPU Memory (GB)",~"GPU Memory (GB)",~"CPU TFLOPS",~"GPU TFLOPs",~"SUs",
               9, "Xeon E5-2650 v3", "2.3", 9*20, 10, 8*128, 80, 5.76, 2.57,180*8760,
               33, "Xeon E5-2670 v3", "2.3", 33*24, 62, 33*128, 496, 25.344, 15.934,792*8760,
               14, "Xeon E5-2650 v4", "2.2", 14*24, 0, 14*64, 0, 9.6768, 0,336*8760,
               1, "Xeon E5-2640 v3", "2.6", 16, 0, 512, 0, 0.5632, 0,16*8760,
               24, "Xeon Gold 6140", "2.3", 24*36, 48, 24*192, 528, 41.472, 18.392,864*8760,
               6, "Xeon Gold 6240", "2.6", 6*36, 0, 6*192, 0, 10.368, 0,216*8760,
               2, "Xeon Gold 6230R", "2.1", 2*52, 0, 2*384, 0, 4.3264, 0, 104*8760,
               1, "Xeon Gold 6348R", "3.0", 48, 5, 192, 5*40, 3.072, 48.5, 48*8760,
               3, "EPYC 7302", "3.0", 3*32, 3*8, 3*256, 3*8*48, 4.3008, 28.08, 3*32*8760 
               )
sollts <- tribble(~"Nodes",~"CPU Type",~"CPU Speed (GHz)",~"CPUs",~"GPUs",~"CPU Memory (GB)",~"GPU Memory (GB)",~"CPU TFLOPS",~"GPU TFLOPs",~"SUs",
               8, "E5-2650 v3", "2.3", 160, 0, 8*128, 0, 736*8/1000, 0,160*8760
               )

totalsol <- sol %>% summarise_if(is.numeric, sum) %>% 
  mutate("CPU Type"="","CPU Speed (GHz)"="") %>% 
  select(Nodes,"CPU Type","CPU Speed (GHz)",CPUs:SUs)

hawk <- tribble(~"Nodes",~"CPU Type",~"CPU Speed (GHz)",~"CPUs",~"GPUs",~"CPU Memory (GB)",~"GPU Memory (GB)",~"CPU TFLOPS",~"GPU TFLOPs",~"SUs",
...
* Login to Sol using a SSH client
    * `ssh <username>@sol.cc.lehigh.edu`
* OR us the Open OnDemand portal and start a Shell on Sol
    * https://hpcportal.cc.lehigh.edu

* Explore your home directory using the `ls command`
* You should see a directory called `hpc2022prog_083122` and within it a directory corresponding to your username
* Change to the `hpc2022prog_083122/<my username directory>`
* Copy Workshop material
    * `cp -r /home/alp514/Workshop/2022HPC/parprog .`
...
* Commercial but available free of charge
   ...
   - Intel OneAPI: 2021.3
* All except gcc 8.3.1 are available via the module environment. 
...
d. You can use either `intel/2021.3.0` or `oneapi/2021.3.0`. Applications built with `ifort/icc/icpc` are provided by `intel/2021.3.0` while those built with `ifx/icx/icpx` are provided by `oneapi/2021.3.0`.
]
...
# For workshop only
#SBATCH --account=hpc2022_qc_083122
...
g16 < water-hexamer-cage-opt-freq.com > water-hexamer-cage-opt-freq.log
...
srun -p hawkgpu -n 1 -N 1 -t 10 -A hpc2022_prog_083122 --reservation=hpc2022_prog_083122_183 &lt;executable&gt;
</pre>
* Submit a job to run on a gpu
<pre class="wrap" style="background-color: var(--white); font-size: 12px">
module load nvhpc
nvcc -o saxpy saxpy.cu
srun -p hawkgpu -n 1 -N 1 -t 10 --gres=gpu:1 -A hpc2022_prog_083122 --reservation=hpc2022_prog_083122_183 ./saxpy
</pre>
* Submit a LAMMPS job or any other of your choice and compare timing results
<pre class="wrap" style="background-color: var(--white); font-size: 12px">
module load hawkgpu
module load lammps/20210310-cuda-11.6.0
srun -p hawkgpu -n 1 -N 1 --gres=gpu:1 -t 10 -A hpc2022_prog_083122 --reservation=hpc2022_prog_083122_183 $(which lmp) -in in.lj -var n 1 -var x 4 -sf gpu -pk gpu 1
</pre>
