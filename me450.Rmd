...
* `rm file1`: delete a file called file1 
  - Unlike other OS&#39;s, once you delete a file it cannot be recoverd.
      - Use `-i` to be prompted for confirmation
...
## Job Types: Interactive
   ...
   - Run a job interactively replace `--pty /bin/bash --login` with the
     appropriate command. 
       - For e.g. `srun -t 20 -n 1 -p im1080 --qos=nogpu $(which lammps) -in in.lj
     -var x 1 -var n 1`
...
| scontrol release jobid | Release the hold that <strong>you put</strong> on <em>jobid |
